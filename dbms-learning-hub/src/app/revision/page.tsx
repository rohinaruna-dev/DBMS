...
export default function RevisionPage() {
  ...
  const go = (direction: number) => {
    ...
    const next = (current + direction + filteredCards.length) % filteredCards.length;
    setDir(direction);
    setCurrent(next);
    const c = filteredCards[next];
    if (!viewed.has(c.id)) {
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      ...
      <Box sx={{ mb: 4 }}>
        <ToggleButtonGroup
          ...
          value={moduleFilter}
          exclusive
          onChange={(e, v) => v && handleModuleChange(v)}
          size="small"
        >
      ...
      <Box sx={{ mb: 3 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
          <Typography variant="body2" fontWeight={600} color="text.secondary">
            Card {filteredCards.length > 0 ? current + 1 : 0} of {filteredCards.length}
          </Typography>
          <Typography variant="body2" fontWeight={700} color="#f59e0b">
            {viewed.size}/{revisionCards.length} reviewed total
          </Typography>
        </Box>
        <LinearProgress
          variant="determinate"
      ...
      {filteredCards.length > 0 && (
        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Button
            ...
            startIcon={<CheckIcon />}
            onClick={markCurrent}
            disabled={card && viewed.has(card.id)}
      ...
      {filteredCards.length > 0 && (
        <Box sx={{ mt: 4 }}>
          <Typography variant="h6" fontWeight={700} sx={{ mb: 2 }}>Topic Selection</Typography>
